...
const { sleep, threatOf, isHostile, TIER, equipBest, bestFood, cancelCurrentTask, getAIDecision } = require("./helpers.cjs");

function startCombatLoop(bot) {
  // Combat check interval: every 2s instead of 500ms to reduce CPU load
  setInterval(async () => {
    const state = bot.botState;
    if (state.isFighting || state.isChasing) return;

    const botPos = bot.entity?.position;
    if (!botPos || !bot.health) return;
    ...
    // Quick flee for bosses or when critically hurt — skip LLM call for speed
    ...
    // Save followTarget BEFORE cancelCurrentTask clears it
    ...
    try {
      ...
      state.isBusy = false;
      bot.pathfinder.setGoal(null);
      // Restore follow target if we were following someone before combat
      if (prevFollow) {
        state.followTarget = prevFollow;
        state.isFollowGoalSet = false; // force follow interval to re-set the goal
      }
    }
  }, 2000);
}
